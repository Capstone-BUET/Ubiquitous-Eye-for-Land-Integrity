...
├── data/
│   ├── raw/
│   │   ├── landsat/                    # USGS Landsat 8 Level-2 scenes
│   │   │   └── LC08_L2SP_137043_*/     # .TIF band files + metadata (.json, .txt, .xml)
│   │   └── sentinel2/                  # Copernicus Sentinel-2 Level-2A scenes
│   │       └── S2A_MSIL2A_*/           # .SAFE directories per acquisition date
│   │           └── GRANULE/
│   │               └── IMG_DATA/
│   │                   ├── R10m/       # B02, B03, B04, B08 (10 m resolution)
│   │                   └── R20m/       # B01, B11, B12, SCL (20 m resolution)
│   │
│   └── processed/
│       └── filtered-sentinel2-dataset/ # Filtered Parquet files (15.44 GB total)
│           ├── data1.parquet           # ~1.46 GB — Region: Rangpur
│           ├── data2.parquet           # Region: Meherpur
│           ├── data3.parquet           # Region: Jessore
│           ├── data4.parquet           # Region: Pabna
│           ├── data5.parquet           # Region: Naogoan
│           ├── data6.parquet           # Region: Dinajpur
│           ├── data7.parquet           # Region: Bogura
│           ├── data8.parquet           # Region: Mymensingh
│           ├── data9.parquet           # Region: Dhaka
│           ├── data10.parquet          # Region: Chattogram
│           ├── data11.parquet          # Region: Feni
│           └── data12.parquet          # Region: (additional tile)
│
├── notebooks/
│   ├── data-preparation.ipynb          # Step 1–3: Band extraction → filter → Parquet export
│   └── capstone-model.ipynb            # Step 4: Multi-model training & evaluation
│
├── saved_models/
│   ├── xgboost.joblib
│   ├── catboost.joblib
...
└────────┬────────────┘     └──────────┬────────────┘
     ...
     1. USGS Landsat 8 — Level-2 Surface Reflectance
      ...
      ----------------------------------------------------------------------
      |                 |  12 tiles across Bangladesh (Rangpur, Meherpur,  |
      |     Coverage    |    Jessore, Pabna, Naogoan, Dinajpur, Bogura,    |
      |                 |    Mymensingh, Dhaka, Chattogram, Feni, and more)|                                     |
      ----------------------------------------------------------------------
